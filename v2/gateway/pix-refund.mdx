...
openapi: POST /api/v2/gateway/pix-refund
---
...
<CodeGroup>
  <Code title="POST">
    ```http
    POST /api/v2/gateway/pix-refund
    ```
  </Code>
</CodeGroup>
...
<CodeGroup>
  <Code title="cURL" active>
    ```bash
    curl -X POST "https://sandbox.ws.relaxpay.site/api/v2/gateway/pix-refund" \
      -H "ci: seu-client-id" \
  ...
  <Code title="Node.js">
    ```javascript
    const response = await fetch('https://sandbox.ws.relaxpay.site/api/v2/gateway/pix-refund', {
      method: 'POST',
      headers: {
        'ci': 'seu-client-id',
  ...
  <Code title="PHP">
    ...
    curl_setopt_array($curl, [
      CURLOPT_URL => "https://sandbox.ws.relaxpay.site/api/v2/gateway/pix-refund",
