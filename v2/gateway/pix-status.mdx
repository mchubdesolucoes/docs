...
openapi: GET /api/v2/gateway/check-payment
---
...
<CodeGroup>
  <Code title="GET">
    ```http
    GET /api/v2/gateway/check-payment
    ```
  </Code>
</CodeGroup>
...
<CodeGroup>
  <Code title="cURL" active>
    ```bash
    curl -X GET "https://sandbox.ws.relaxpay.site/api/v2/gateway/check-payment?idTransaction=0b935d10-3c90-4af3-8776-dd18622a7941" \
      -H "ci: seu-client-id" \
  ...
  <Code title="Node.js">
    ...
    const response = await fetch(
      'https://sandbox.ws.relaxpay.site/api/v2/gateway/check-payment?idTransaction=0b935d10-3c90-4af3-8776-dd18622a7941',
      {
        method: 'GET',
        headers: {
          'ci': 'seu-client-id',
  ...
  <Code title="PHP">
    ...
    curl_setopt_array($curl, [
      CURLOPT_URL => "https://sandbox.ws.relaxpay.site/api/v2/gateway/check-payment?idTransaction=0b935d10-3c90-4af3-8776-dd18622a7941",
...
<CodeGroup>
  <Code title="Node.js">
    ...
    async function checkPaymentStatus(idTransaction) {
      try {
        const response = await fetch(
          `https://sandbox.ws.relaxpay.site/api/v2/gateway/check-payment?idTransaction=${idTransaction}`,
          {
            method: 'GET',
            headers: {
