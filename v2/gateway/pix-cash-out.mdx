...
openapi: POST /api/v2/gateway/pix-payment
---
...
<CodeGroup>
  ...
  <Code title="cURL">
    ```bash
    curl -X POST https://ws.relaxpay.site/api/v2/gateway/pix-payment \
      -H "ci: seu-client-id" \
  ...
  <Code title="JavaScript">
    ```javascript
    const response = await fetch('https://ws.relaxpay.site/api/v2/gateway/pix-payment', {
      method: 'POST',
      headers: {
        'ci': 'seu-client-id',
